...
issue #2879 描述了完整问题空间，包括 sandbox colocation and hardening、high-concurrency scheduling optimization、sandbox pipeline、automated diagnostics and feedback。本 proposal 第一版不把四个方向一次性展开，而是先收敛到社区沟通中确认的一期重点：

1. **接入闭环**：Pod sandbox 直接进入 koord-scheduler；非 Pod sandbox 通过 Reservation 或节点级 NodeReservation 配置进入 Koordinator 资源账本。
2. **快速调度**：为明确 opt-in 的 sandbox workload 建立轻量调度路径，并围绕 5000 QPS 绑定能力目标拆解 queue、filter / score、reserve、bind、APIServer / etcd 写入等瓶颈。

核心判断是：Koordinator 不新增 Sandbox CRD，也不替代上游 sandbox API。只要 sandbox API 最终创建 Kubernetes Pod，就以 Pod / PodTemplate 作为接入边界；如果 sandbox 消耗 Koordinator 管理节点上的资源但不创建 Pod，则由 adapter / glue layer 将外部 allocation 映射为 Koordinator Reservation，或在粗粒度场景更新节点级 NodeReservation 配置。预热、QoS 深度接入、sandbox 故障感知与诊断保留 TBD 位置，不作为本次 proposal 的主线设计。

## 术语

只保留本 proposal 主线会直接使用的术语：

- **Sandbox**：Agent 执行代码、工具或评测任务时使用的隔离执行环境。在 Kubernetes 中可能表现为 Pod，也可能由外部 runtime / controller 在节点上创建非 Pod 形态的实例。
- **Reservation 转换层**：把上游 sandbox API 或非 Pod sandbox allocation 转成 Koordinator 可理解的 Pod、Reservation 或节点级 reservation 元数据的 adapter 层。
- **Sandbox fast path**：只对明确标记的 sandbox workload 生效的轻量调度路径，用于减少通用调度 pipeline 中不必要的计算和写入。

## 动机

issue #2879 的核心问题不是 Koordinator 要重新定义 AI Agent 编排模型，而是 Koordinator 可以把已有调度能力用于 AI sandbox 场景，解决上层系统难以单独处理的调度问题。

Koordinator 在这个问题上有几类天然优势：

- **Reservation 能承接非 Pod 形态的资源记账**：很多 sandbox API 最终会落成 Pod，可以直接进入 koord-scheduler；但仍可能存在 node-local VM、host process、Wasm runtime 等非 Pod 形态。只要它们消耗 Koordinator 管理节点上的资源，Reservation 可以作为对象级转换层；粗粒度、相对静态的节点扣减可以使用 NodeReservation 配置，避免资源不可见。
...
- **QoS 能力可以作为后续承接面**：sandbox 可能使用不同 runtime、不同优先级、不同资源隔离要求。Koordinator 已有 QoS class 和 `ClusterColocationProfile`，可以在接入闭环稳定后继续承接 QoS 分类与隔离增强，但一期不深改 QoS。
...
因此，本 proposal 的动机是把 Koordinator 的 Reservation 和调度 pipeline 连接成一条面向 sandbox 的接入与快速调度路径；一期先打通接入和性能基线，预热、QoS 深度接入、复杂诊断在后续阶段补齐。
...
- 重点设计非 Pod sandbox 到 Reservation 或节点级 NodeReservation 配置的胶水层，使 Koordinator 能感知这类 sandbox 的资源占用。
...
- 不新增 Sandbox CRD；Koordinator 适配上游 sandbox API，并把它们接入现有 Pod、Reservation 和节点级 reservation 调度面。
- 不实现 sandbox runtime、网络隔离、文件系统隔离或 VM 生命周期管理。
- 不在本次 proposal 深入设计 warm capacity、睡眠唤醒、QoS 深度接入或复杂自动诊断；这些能力只保留 TBD 位置，后续单独展开状态机、策略和 API。
...
外部 runtime 在 Koordinator 管理节点上创建 node-local VM 或 host process sandbox，但不创建 Kubernetes Pod。adapter 需要把这类 allocation 映射为对象级 Reservation；粗粒度、相对静态的节点扣减可以使用 NodeReservation 配置。目标是保证调度账本不超卖，并在 sandbox 退出后释放账本。

**Story 3：SWE-bench 风格评测**

评测平台一次启动数百个短生命周期 sandbox Pod。这些 Pod 使用相同镜像和相似资源规格。调度器应该避免为每个 sandbox 重复执行相同 filter / score 计算，并以有界延迟完成绑定。
...
- **现有调度面优先**：Pod 形态走 Pod fast path；非 Pod 通过 Reservation 或节点级 NodeReservation 配置表达；warm capacity 作为后续扩展评估。
- **Opt-in**：sandbox-specific 调度行为只作用于明确标记的 sandbox workload，不影响普通 Pod。
...
| P1 / 接入闭环 | Pod sandbox 通过 label / schedulerName 接入；非 Pod sandbox 通过 Reservation 或节点级 NodeReservation 配置记账；跑通 sandbox 创建、调度、启动 / 记账、释放。 | 第一阶段先证明 sandbox 可以通过 Koordinator 跑起来。 | 预热、QoS 接入、复杂诊断。 |
...
| P3 / Bind 与 API 写入优化 | 在调研 Kubernetes bind、Koordinator Reservation bind、Volcano agent-scheduler binder 等实现后，落地 bind / API 写入优化。 | 5000 QPS 目标的瓶颈很可能出现在 binding cycle、对象 patch、APIServer / etcd 写入。 | 全局并发调度。 |
...
Koordinator 应把 sandbox support 设计为围绕现有 Pod、Reservation、NodeReservation 和调度扩展机制的一组集成。
...
- **NodeReservation**：适用于粗粒度、相对静态的节点级资源扣减。它是节点 annotation 承载的 node-level reservation 配置，不是和 Reservation 同级的对象级调度 CRD，因此不适合作为大量短生命周期 sandbox 的唯一模型。
...
- Koordinator NodeReservation 通过节点 annotation `node.koordinator.sh/reservation` 表达静态或相对静态的节点级保留资源，可以影响节点可调度资源或只保留 CPU cores。它适合 coarse-grained node-level deduction，不适合表达 per-sandbox、高 churn 的对象生命周期。
...
第一阶段目标建议优先选择少量重点 API，例如 `kubernetes-sigs/agent-sandbox`。issue 讨论中明确提出需要补充 Sandbox API awareness；本地代码确认该项目的 Sandbox 路径最终会从 PodTemplate 创建 Kubernetes Pod，因此 Pod / PodTemplate 是 Koordinator 最稳妥的集成边界。

API awareness 层应该：

- 识别最终会创建 Pod 的 sandbox 对象。
- 将 Koordinator 调度元数据注入或透传到最终 Pod / PodTemplate。
- 对非 Pod sandbox，只在其消耗 Koordinator 管理节点资源且存在明确 adapter 时，映射为 Reservation 或节点级 NodeReservation 配置。
...
| 在 Koordinator 管理节点上，但不是 Pod | node-local Firecracker VM、host process sandbox、外部 runtime allocation | 是，但需要 adapter | Reservation 记账，或用 NodeReservation 做粗粒度节点扣减；是否由 Koordinator 选 node 需要单独协议。 |

第一版接入路径只分两类：

| Runtime shape | First-stage path | Notes |
|---|---|---|
| Pod-shaped sandbox | Pod 或 PodTemplate 打稳定 label / annotation，必要时由 webhook / adapter 注入 `schedulerName: koord-scheduler`，直接走 sandbox fast path。 | `kubernetes-sigs/agent-sandbox`、gVisor、Kata 等最终落成 Pod 的路径都属于这一类。 |
| Non-Pod sandbox | adapter 创建不被普通 Pod 认领的 accounting-only Reservation，或在粗粒度场景使用 NodeReservation 做节点级扣减。 | 只接入运行在 Koordinator 管理节点上、且资源占用能被 adapter 明确上报的 allocation。 |
...
阶段性目标是以 5000 QPS 绑定能力作为 benchmark 牵引，但这个数值不是 API 语义保证，而是用来拆解 queue、filter / score、reserve、bind、APIServer 写入等瓶颈。
...
- sandbox workload 使用专门 label / annotation opt in。
...
- 非 Pod sandbox 通过 Reservation 完成资源占位，或通过 NodeReservation 完成粗粒度节点扣减，不引入新的 sandbox 调度对象。
...
|---|---|
| B1.1 | 实现 sandbox fast path opt-in 机制：label / annotation、`schedulerName`、scheduler profile。 |
...
部分部署可能运行 node-local sandbox：它们消耗 Koordinator 管理节点上的资源，但不创建 Pod。这不是一期主路径，但账本模型不应阻塞后续支持。adapter 可以通过 Reservation 上报对象级 allocation，或通过 NodeReservation annotation 做粗粒度节点扣减：

- 外部 allocation 是 per-sandbox、高 churn、需要对象级生命周期时，使用 Reservation。
- 只有粗粒度、相对静态的节点级扣减时，更新 NodeReservation annotation。
...
- **Non-Pod accounting**：adapter 根据外部 allocation 创建 accounting-only Reservation，或更新节点级 NodeReservation 配置。Reservation 对象应带上外部 sandbox id、runtime、resource shape、nodeName、生命周期状态等 label / annotation，便于审计和清理。
...
- accounting-only Reservation 或 NodeReservation 配置使用外部 sandbox 实际占用的资源口径。
...
Phase 3 初版可以只让少量插件 opt in。保守的部分缓存可以接受；不安全的全局复用不可接受。
...
|---|---|---|
| M1: Reservation conversion and Pod classification | 定义 sandbox labels / annotations、重点上游 API adapter 行为、非 Pod accounting-only Reservation 语义和 NodeReservation 使用边界。 | PodTemplate-based sandbox 能被 Koordinator 识别并路由到 `koord-scheduler`；非 Pod 接入边界和记账协议被文档化。 |
| M2: Accounting closure | 验证 Reservation template、NodeReservation 配置或非 Pod adapter 上报的资源口径正确；sandbox 退出后账本释放或节点级扣减被正确更新。 | sandbox Reservation 或 NodeReservation 配置不会因为记账口径不一致或释放失败造成节点超卖。 |
| M3: Minimal fast path | 建立 sandbox opt-in scheduler profile / fast path，并明确最小插件集合。 | Pod-shaped sandbox 能走 fast path，普通 Pod 不受影响。 |
| M4: 5000 QPS benchmark | 建立 5000 QPS 绑定能力 benchmark，拆分 queue、filter / score、reserve、prebind、bind、APIServer 写入。 | 能定位下一阶段瓶颈，且 benchmark 可重复运行。 |
...
NodeReservation 适合静态节点级扣减，但 per-sandbox、高 churn 的外部 allocation 需要对象级生命周期。Reservation 更适合表达这类细粒度账本。
...
- 后续阶段：equivalence-class key generation、cache invalidation 和 plugin opt-in 行为；
