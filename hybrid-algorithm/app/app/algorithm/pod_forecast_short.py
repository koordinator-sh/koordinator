...
# ================== 参数 ==================
INPUT_JSONL = "/root/autodl-tmp/BurstData/performance_with_resource_metrics_with_burst.jsonl"
OUT_DIR = "/root/autodl-tmp/BurstData/ForecastOutputWorkloadLSTM"

BUCKET_SEC = 180         # ← 修正：数据采样间隔是180秒，原来60会导致大量空桶
IN_LEN = 30              # 输入窗口：前30桶（= 90分钟）历史
OUT_LEN = 30             # 预测窗口：未来30桶（= 90分钟）
...
def pass2_build_workload_series(input_jsonl, mem_limit, cpu_quota, cpu_period):
    ...
    container_io_raw  = defaultdict(list)   # (ts, bytes_read+write累计)
    container_net_raw = defaultdict(list)   # (ts, bytes_rx+tx累计)
    container_meta = {}
    ...
    wl_io  = defaultdict(lambda: defaultdict(float))   # bucket -> mbps
    ...
    if cpu_skipped > 0:
        print(f"  ⚠ CPU: {cpu_skipped} 个容器因缺少 quota/period 被跳过（CPU序列将全为0）")
    ...
    # 处理 IO 容器（reads+writes counter，计算速率 MB/s）
    def _agg_counter_mbps(raw_dict, wl_target):
        for ck, raw_pairs in raw_dict.items():
            ...
            for i in range(len(raw_pairs) - 1):
                ...
                delta = max(v1 - v0, 0.0)
                duration = t1 - t0
                mbps = delta / duration / (1024 * 1024)   # bytes/s -> MB/s
                m = t0 // BUCKET_SEC
                wl_target[wl][m] += mbps
                if pod: wl_pods[wl][m].add(pod)
...
def train_lstm_one_step(series_2d, cutoff_idx=None, batch_size=64, epochs=EPOCHS, patience=5, max_samples=2000):
    ...
    with torch.no_grad():
        for _ in range(mc_samples):
            ...
            for _ in range(out_len):
                raw = model(x).squeeze(0).cpu().numpy().tolist()
                # CPU/MEM 归一化到 [0,1]，IO/NET 只保证非负
                y_next = [
                    clamp01(raw[IDX_CPU]),
                    clamp01(raw[IDX_MEM]),
                    max(0.0, raw[IDX_IO]),
                ...
                x = torch.cat([x[:, 1:, :], torch.tensor([[y_next]], dtype=torch.float32, device=DEVICE)], dim=1)
...
def backtest_workload(wl, namespace, series_2d, wl_pods, min_m, max_m, n_windows):
    ...
    for k in range(n_windows):
        ...
        pred_cpu_mean = [mean[i][IDX_CPU] for i in range(OUT_LEN)]
        pred_mem_mean = [mean[i][IDX_MEM] for i in range(OUT_LEN)]
        pred_io_mean  = [mean[i][IDX_IO]  for i in range(OUT_LEN)]
        pred_net_mean = [mean[i][IDX_NET] for i in range(OUT_LEN)]
        ...
        cpu_mae = sum(abs(pred_cpu_mean[i]-gt_cpu[i]) for i in range(OUT_LEN))/OUT_LEN
        mem_mae = sum(abs(pred_mem_mean[i]-gt_mem[i]) for i in range(OUT_LEN))/OUT_LEN
        io_mae  = sum(abs(pred_io_mean[i] -gt_io[i])  for i in range(OUT_LEN))/OUT_LEN
        net_mae = sum(abs(pred_net_mean[i]-gt_net[i]) for i in range(OUT_LEN))/OUT_LEN
...
def main():
    ...
    if total == 0:
        ...
        backtest_summary = [("警告", f"数据不足，至少需要 {IN_LEN + OUT_LEN*(BACKTEST_WINDOWS+1)} 桶序列")]
    else:
    ...
    write_forecast_csv(os.path.join(OUT_DIR, "workload_forecast_90m_lstm.csv"), forecast_rows)
    write_alert_reco_csv(os.path.join(OUT_DIR, "workload_alerts_and_scale_pods_90m_lstm.csv"), alert_rows)
    ...
    print("\n===== DONE =====")
    print("实际预测结果:    ", os.path.join(OUT_DIR, "workload_forecast_90m_lstm.csv"))
    print("告警+扩容建议:   ", os.path.join(OUT_DIR, "workload_alerts_and_scale_pods_90m_lstm.csv"))
