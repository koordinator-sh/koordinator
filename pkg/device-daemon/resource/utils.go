...
func GetDeviceInfo(deviceType, index string) (koordletuti.DeviceTopology, koordletuti.DeviceStatus, string, error) {
	...
	case NPU:
		...
		output, err := cmd.CombinedOutput()
		if err != nil {
			klog.Errorf("GetDeviceInfo, failed to get npu device info: %v, output: %s", err, string(output))
		...
		output, err = cmd.CombinedOutput()
		if err != nil {
			klog.Errorf("GetDeviceInfo, failed to get npu topo: %v, output: %s", err, string(output))
		}
