...
func TestInternalRegistryGathersAllInternalMetrics(t *testing.T) {
	...
	t.Cleanup(func() { resetCollectors(t, internalCollectorSlices) })
...
var (
	...
	registrationProbe = k8smetrics.NewGaugeVec(&k8smetrics.GaugeOpts{
		...
		Help:           "test-only metric asserting internalKubeMustRegister publishes to InternalRegistry",
...
// TestInternalKubeMustRegisterPublishesToInternalRegistry exercises the Registerable registration path that the
...
func TestInternalKubeMustRegisterPublishesToInternalRegistry(t *testing.T) {
	registrationProbeOnce.Do(func() { internalKubeMustRegister(registrationProbe) })
	...
	assert.True(t, gathered["koordlet_registration_probe"],
		"a component-base metric registered via internalKubeMustRegister is not reported by InternalRegistry")
}

// resetCollectors clears every series held by the given collectors, so later tests in the package do not
