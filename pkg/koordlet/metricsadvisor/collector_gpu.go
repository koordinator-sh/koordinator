...
func (g *gpuDeviceManager) getNodeGPUUsage() []metriccache.GPUMetric {
	...
	for i := 0; i < g.deviceCount; i++ {
		rtn[i] = metriccache.GPUMetric{
			...
			TotalMemory: *resource.NewQuantity(int64(g.devices[i].TotalMemory), resource.BinarySI),
		}
	}
	return rtn
}
...
func (g *gpuDeviceManager) getPodGPUUsage(podParentDir string, cs []corev1.ContainerStatus) ([]metriccache.GPUMetric, error) {
	runingContainer := make([]corev1.ContainerStatus, 0)
	for i := range cs {
		if cs[i].State.Running == nil {
			klog.V(6).Infof("non-running container %s", cs[i].ContainerID)
			continue
		}
		runingContainer = append(runingContainer, cs[i])
	}
	if len(runingContainer) == 0 {
		return nil, nil
	}
...
func (g *gpuDeviceManager) getContainerGPUUsage(podParentDir string, c *corev1.ContainerStatus) ([]metriccache.GPUMetric, error) {
	if c.State.Running == nil {
		klog.V(6).Infof("non-running container %s", c.ContainerID)
		return nil, nil
	}
