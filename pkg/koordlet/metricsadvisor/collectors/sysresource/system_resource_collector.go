...
func (s *systemResourceCollector) Setup(c *framework.Context) {
	...
	// ponytail: log once at setup, not every collect cycle
	if _, ok := c.Collectors["PageCacheCollector"]; ok {
		s.pageCacheCollectorEnabled = true
	}
}
	...
	if s.statesInformer != nil {
		...
		if nodeMetricSpec != nil && nodeMetricSpec.CollectPolicy != nil && nodeMetricSpec.CollectPolicy.NodeMemoryCollectPolicy != nil {
			...
			if policy == slov1alpha1.UsageWithPageCache || policy == slov1alpha1.UsageWithHotPageCache {
				if !s.pageCacheCollectorEnabled {
					klog.Warning("NodeMemoryCollectPolicy is set to UsageWithPageCache or UsageWithHotPageCache but page-cache collector is not enabled, policy will have no effect")
				}
