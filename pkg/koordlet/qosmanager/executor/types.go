...
func CommonUpdateFunc(resource ResourceUpdater) error {
	...
	audit.V(5).Node().Reason(reason.UpdateSystemConfig).Message("update %v to %v", info.file, info.value).Do()
...
func CommonCgroupUpdateFunc(resource ResourceUpdater) error {
	...
	if info.owner != nil {
		...
		case podType:
			audit.V(5).Pod(info.owner.Namespace, info.owner.Name).Reason(reason.UpdateCgroups).Message("update %v to %v", info.file, info.value).Do()
		case containerType:
			audit.V(5).Pod(info.owner.Namespace, info.owner.Name).Container(info.owner.Container).Reason(reason.UpdateCgroups).Message("update %v to %v", info.file, info.value).Do()
		case nodeType:
			audit.V(5).Node().Reason(reason.UpdateCgroups).Message("update %v to %v", info.file, info.value).Do()
		case groupType:
			audit.V(5).Group(info.owner.Name).Reason(reason.UpdateCgroups).Message("update %v to %v", info.file, info.value).Do()
		default:
			audit.V(5).Unknown(info.owner.Name).Reason(reason.UpdateCgroups).Message("update %v to %v", info.file, info.value).Do()
		}
	}
...
func updateResctrlSchemataFunc(resource ResourceUpdater) error {
	...
	// MB:0=100;1=100
	info := resource.(*CommonResourceUpdater)
	audit.V(5).Group(info.owner.Name).Reason(reason.UpdateResctrlSchemata).Message("update %v with value %v",
...
func updateResctrlTasksFunc(resource ResourceUpdater) error {
	...
	// 124
	info := resource.(*CommonResourceUpdater)
	audit.V(5).Group(info.owner.Name).Reason(reason.UpdateResctrlTasks).Message("update %v with value %v",
		resource.Key(), resource.Value()).Do()
