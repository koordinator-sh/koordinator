...
import (
	...
	"github.com/koordinator-sh/koordinator/pkg/koordlet/qosmanager/executor"
...
const DefaultReconcileIntervalInSeconds = 5
const DefaultCollectMetricsIntervalSeconds = int64(1)
...
func New(pluginCtx *plugins.PluginContext) plugins.Plugin {
	...
	if pluginCtx.ExtraConfig != nil {
		if err := json.Unmarshal([]byte(*pluginCtx.ExtraConfig), &cfg); err != nil {
			klog.Errorf("[Plugin: %v] failed to parse extra config: %s", cpuBurst.Name(), *pluginCtx.ExtraConfig)
		}
	}
	...
	cpuBurst.cfg = &cfg
	cpuBurst.executor = executor.NewResourceUpdateExecutor("CPUBurstExecutor", cfg.ReconcileIntervalInSeconds*60)

	return cpuBurst
}

// Name impl interface Plugin.
func (_ *CPUBurst) Name() string {
	return "CPUBurst"
}

// Name impl interface Plugin.
func (_ *CPUBurst) Feature() featuregate.Feature {
	return features.CPUBurst
}

// Start impl interface Plugin.
func (r *CPUBurst) Start() error {
	r.stopCh = make(chan struct{})
	go wait.Until(r.start, time.Duration(r.cfg.ReconcileIntervalInSeconds)*time.Second, r.stopCh)
	return nil
}

// Stop impl interface Plugin.
func (r *CPUBurst) Stop() error {
	if r.stopCh != nil {
		close(r.stopCh)
		r.stopCh = nil
	}
...
//}
//
//func New(pluginCtx *plugins.PluginContext) *CPUBurst {
...
func (b *CPUBurst) getNodeStateForBurst(sharePoolThresholdPercent int64,
	...
	overloadMetricDurationSeconds := util.MinInt64(int64(b.cfg.ReconcileIntervalInSeconds*5), 10)
	queryParam := metricsquery.GenerateQueryParamsAvg(overloadMetricDurationSeconds)
	nodeMetric, podsMetric := b.metricsQuery.CollectNodeAndPodMetrics(queryParam)
	if nodeMetric == nil {
		klog.Warningf("node metric is nil during handle cfs burst scale down")
		return nodeBurstUnknown
	}
	nodeCPUInfo, err := b.metricCache.GetNodeCPUInfo(&metriccache.QueryParam{})
...
func (b *CPUBurst) cfsBurstAllowedByLimiter(burstCfg *slov1alpha1.CPUBurstConfig, container *corev1.Container,
	...
	containerRes := b.metricsQuery.CollectContainerResMetricLast(containerID, b.cfg.CollectMetricsIntervalSeconds)
...
func (b *CPUBurst) genOperationByContainer(burstCfg *slov1alpha1.CPUBurstConfig, pod *corev1.Pod,
	...
	containerThrottled := b.metricsQuery.CollectContainerThrottledMetricLast(&containerStat.ContainerID, b.cfg.CollectMetricsIntervalSeconds)
