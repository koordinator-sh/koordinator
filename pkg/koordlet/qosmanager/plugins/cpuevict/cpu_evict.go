...
func (c *cpuEvictor) cpuEvict() {
	...
	for _, feature := range triggerFeatures {
		if disabled, err := features.IsFeatureDisabled(nodeSLO, feature); err != nil {
			...
			klog.V(4).Infof("feature %s failed, nodeSLO disable the feature gate", feature)
			continue
		}
...
func (c *cpuEvictor) buildEvictTask(feature featuregate.Feature, nodeSLO *slov1alpha1.NodeSLO, nodeMilliCPUCapacity int64) (*qosmanagerUtil.EvictTaskInfo, error) {
	...
	var calculateReleaseReource func(*slov1alpha1.ResourceThresholdStrategy, int64) int64
	// check config
	switch feature {
	case features.BECPUEvict:
		evictReason = resourceexecutor.EvictBEPodByBECPUSatisfaction
		configCheckFunc = isSatisfactionConfigValid
		calculateReleaseReource = c.calculateMilliReleaseBySatisfaction
	...
	case features.CPUEvict:
		evictReason = resourceexecutor.EvictPodByCPUUsedThresholdPercent
		configCheckFunc = isUsedThresholdConfigValid
		calculateReleaseReource = c.calculateMilliReleaseByUsedThresholdPercent
	...
	milliRelease := calculateReleaseReource(thresholdConfig, nodeMilliCPUCapacity)
	if milliRelease <= 0 {
		return nil, nil
	}
...
func (c *cpuEvictor) getPodEvictInfoAndSortByPriority(thresholdConfig *slov1alpha1.ResourceThresholdStrategy) []*qosmanagerUtil.PodEvictInfo {
	...
	for _, podMeta := range c.statesInformer.GetAllPods() {
		klog.Infof("[jxtest] filter pod %v", podMeta.Pod.Name)
		pod := podMeta.Pod
		...
		klog.Infof("[jxtest] filter pod %v,LabelPriority %v", podMeta.Pod.Name, podPriority)
		// 4. filter no metrics
		...
		klog.Infof("[jxtest] filter pod %v,MilliCPUUsed %v", podMeta.Pod.Name, podInfo.MilliCPUUsed)
		...
		klog.Infof("[jxtest] filter pod %v,milliRequestSum %v", podMeta.Pod.Name, milliRequestSum)
