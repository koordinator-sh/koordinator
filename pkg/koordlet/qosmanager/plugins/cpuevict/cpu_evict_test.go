	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			pluginCtx := &plugins.PluginContext{
				...
				ExtraConfig: pointer.StringPtr(`{"reconcileIntervalInSeconds": 5, "collectMetricsIntervalSeconds": 1, "cpuEvictCoolTimeSeconds": 20}`),
			}
			cpuEvictor := New(pluginCtx).(*CPUEvictor)
...
func Test_getPodEvictInfoAndSort(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockStatesInformer.EXPECT().GetAllPods().Return(utils.GetPodMetas(tt.pods)).AnyTimes()
...
func Test_killAndEvictBEPodsRelease(t *testing.T) {
	...
	mockStatesInformer.EXPECT().GetNodeSLO().Return(&slov1alpha1.NodeSLO{ObjectMeta: metav1.ObjectMeta{Name: node.Name}}).AnyTimes()

	pluginCtx := &plugins.PluginContext{
		K8sClient:      k8s.NewK8sClient(client, fakeRecorder),
		StatesInformer: mockStatesInformer,
		ExtraConfig:    pointer.StringPtr(`{"reconcileIntervalInSeconds": 5, "collectMetricsIntervalSeconds": 1, "cpuEvictCoolTimeSeconds": 20}`),
	}
	cpuEvictor := New(pluginCtx).(*CPUEvictor)
	...
	cpuEvictor.lastEvictTime = time.Now().Add(-5 * time.Minute)
	_ = cpuEvictor.Start()
