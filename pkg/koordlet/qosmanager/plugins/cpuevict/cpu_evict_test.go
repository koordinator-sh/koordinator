...
func Test_CPUEvict_calculateMilliReleaseBySatisfaction(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gotRelease, _ := c.calculateMilliReleaseBySatisfaction(tt.thresholdConfig, 0, tt.currentNode)
			if len(gotRelease) == 0 {
...
func Test_getBEPodEvictInfoAndSort(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := cpuEvictor.getBEPodEvictInfoAndSort(string(features.BECPUEvict), nil)
	...
	tests := []struct {
		...
		},
		{
			...
			thresholdConfig: &slov1alpha1.ResourceThresholdStrategy{
				...
				AllocatableEvictPriorityThreshold:   ptr.To[int32](apiext.PriorityProdValueDefault),
			},
			expectErr: fmt.Errorf("priorityThresholdPercent(9500) should less than 7999, koor-prod pods should not be killed"),
		},
		{
...
func mockPodForCPUEvictWithPriority(name string, qosClass apiext.QoSClass, priority int32, request, limits corev1.ResourceList) *corev1.Pod {
	return &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			Namespace: "test-ns",
			Name:      name,
...
func Test_calculateReleaseByUsedThresholdPercent(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			res, _ := CPUEvictor.calculateMilliReleaseByUsedThresholdPercent(tt.thresholdConfig, nodeMilliCPUCapacity, tt.node)
			if len(res) == 0 {
				assert.Equal(t, tt.expectRelease, int64(0), "checkRelease")
			} else {
...
func Test_calculateMilliReleaseByAllocatableThresholdPercent(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			pods: []*corev1.Pod{
				mockNonBEPodForCPUEvict("batch-pod-1", apiext.QoSBE, 20000),
				mockNonBEPodForCPUEvict("batch-pod-2", apiext.QoSBE, 30000),
			},
			thresholdConfig: &slov1alpha1.ResourceThresholdStrategy{
				Enable:                              ptr.To[bool](true),
				CPUAllocatableEvictThresholdPercent: ptr.To[int64](80),
		...
		},
		{
			...
			pods: []*corev1.Pod{
				mockBEPodForCPUEvict("batch-pod-1", 45000, 100),
				mockBEPodForCPUEvict("batch-pod-2", 40000, 100),
			},
			thresholdConfig: &slov1alpha1.ResourceThresholdStrategy{
				Enable:                              ptr.To[bool](true),
				CPUAllocatableEvictThresholdPercent: ptr.To[int64](80),
		...
		},
		{
			...
			pods: []*corev1.Pod{
				mockBEPodForCPUEvict("batch-pod-1", 85000, 100),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			nodeMilliCPUCapacity := tt.node.Status.Capacity.Cpu().MilliValue()
			res, calculateFunc := CPUEvictor.calculateMilliReleaseByAllocatableThresholdPercent(tt.thresholdConfig, nodeMilliCPUCapacity, tt.node)
...
// todo: add allocatable evict  test
