...
func (r *CPUSuppress) adjustByCPUSet(cpusetQuantity *resource.Quantity, nodeCPUInfo *metriccache.NodeCPUInfo) {
	...
	audit.V(1).Node().Reason(reason.AdjustBEByNodeCPUUsage).Message("update BE group to cpuset: %v", beCPUSet).Do()
...
func adjustByCfsQuota(cpuQuantity *resource.Quantity, node *corev1.Node) {
	...
	audit.V(1).Node().Reason(reason.AdjustBEByNodeCPUUsage).Message("update BE group to cfs_quota: %v", newBeQuota).Do()
