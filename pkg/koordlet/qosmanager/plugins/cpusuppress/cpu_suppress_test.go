...
import (
	...
	"github.com/koordinator-sh/koordinator/pkg/koordlet/common/types"
...
func Test_cpuSuppress_suppressBECPU(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				...
				nodeCPUSet:    "0-15",
				preBECPUSet:   "0-9",
				preBECFSQuota: 16 * types.DefaultCFSPeriod,
				thresholdConfig: &slov1alpha1.ResourceThresholdStrategy{
			...
			wantBECFSQuota:           16 * types.DefaultCFSPeriod,
		...
		},
		{
			...
			args: args{
				...
				nodeCPUSet:    "0-15",
				preBECPUSet:   "0-9",
				preBECFSQuota: 10 * types.DefaultCFSPeriod,
				thresholdConfig: &slov1alpha1.ResourceThresholdStrategy{
					Enable:                      pointer.BoolPtr(true),
					CPUSuppressPolicy:           slov1alpha1.CPUCfsQuotaPolicy,
					CPUSuppressThresholdPercent: pointer.Int64Ptr(70),
			...
			wantBECFSQuota:           3.2 * types.DefaultCFSPeriod,
		...
		},
		{
			...
			args: args{
				...
				nodeCPUSet:    "0-15",
				preBECPUSet:   "0-9",
				preBECFSQuota: 15 * types.DefaultCFSPeriod,
				thresholdConfig: &slov1alpha1.ResourceThresholdStrategy{
					Enable:                      pointer.BoolPtr(true),
					CPUSuppressPolicy:           slov1alpha1.CPUCfsQuotaPolicy,
					CPUSuppressThresholdPercent: pointer.Int64Ptr(70),
			...
			wantBECFSQuota:           1.2 * types.DefaultCFSPeriod,
			wantCFSQuotaPolicyStatus: &policyUsing,
			wantBECPUSet:             "0-15",
			wantCPUSetPolicyStatus:   &policyRecovered,
		},
		{
			...
			args: args{
				...
				nodeCPUSet:    "0-15",
				preBECPUSet:   "0-9",
				preBECFSQuota: 8 * types.DefaultCFSPeriod,
				thresholdConfig: &slov1alpha1.ResourceThresholdStrategy{
					Enable:                      pointer.BoolPtr(true),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			helper.WriteCgroupFileContents(util.GetKubeQosRelativePath(corev1.PodQOSBestEffort), system.CPUCFSPeriod, strconv.FormatInt(types.DefaultCFSPeriod, 10))
			...
			pluginCtx := &plugins.PluginContext{
				...
				MetricsQuery:   metricsquery.NewMetricsQuery(mockMetricCache, si),
				ExtraConfig:    pointer.StringPtr(`{"reconcileIntervalInSeconds": 5, "collectMetricsIntervalSeconds": 1}`),
			}

			cpuSuppress := New(pluginCtx).(*CPUSuppress)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			pluginCtx := &plugins.PluginContext{
				...
				MetricCache:    mockMetricCache,
				MetricsQuery:   metricsquery.NewMetricsQuery(mockMetricCache, mockStatesInformer),
				ExtraConfig:    pointer.StringPtr(`{"reconcileIntervalInSeconds": 5, "collectMetricsIntervalSeconds": 1}`),
			}
			cpuSuppress := New(pluginCtx).(*CPUSuppress)
