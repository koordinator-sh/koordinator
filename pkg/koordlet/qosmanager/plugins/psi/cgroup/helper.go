...
func QuotaFromWeight(weight uint64) *CpuQuota {
	...
	// Note: cm.QuotaPeriod and cm.MilliCPUToCPU are guaranteed to be non-zero on Linux
	// but may be 0 on other platforms; handle this gracefully
...
// cpuWeightFromShares converts cpu shares to weight in range [1, 10000]
...
func GetMilliCPU(weight uint64) int64 {
	...
	// Note: cm.MilliCPUToCPU and cm.SharesPerCPU are guaranteed to be non-zero on Linux
	// but may be 0 on other platforms; handle this gracefully
	milliCPUToCPU := int64(cm.MilliCPUToCPU)
