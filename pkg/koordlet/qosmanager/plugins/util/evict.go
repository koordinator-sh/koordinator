...
var customExecutorInitializer func(evictor *framework.Evictor, onlyEvictByAPI bool) EvictionExecutor
...
func (d *DefaultEvictionExecutor) Evict(pod *corev1.Pod, node *corev1.Node, releaseReason string, message string) bool {
	if d.OnlyEvictByAPI {
		if d.Evictor.EvictPodIfNotEvicted(pod, node, releaseReason, message) {
			return true
		}
	} else {
...
func SetCustomEvictionExecutorInitializer(initializer func(*framework.Evictor, bool) EvictionExecutor) {
	customExecutorInitializer = initializer
}

func InitializeEvictionExecutor(evictor *framework.Evictor, onlyEvictByAPI bool) EvictionExecutor {
...
func KillAndEvictPods(evictionExecutor EvictionExecutor, node *corev1.Node, tasks []*EvictTaskInfo) (ReleaseList, bool) {
	...
	for _, task := range tasks {
		...
		for _, info := range podInfos {
			...
			successEvict := evictionExecutor.Evict(info.Pod, node, releaseReason, fmt.Sprintf("%v, kill pod: %v", task.Reason, info.Pod.Name))
			if successEvict {
				klog.V(5).Infof("%v pick pod %s to evict", releaseReason, podKey)
				...
				klog.V(5).Infof("%v pick pod %s to evict, but failed", releaseReason, podKey)
			}
		}
