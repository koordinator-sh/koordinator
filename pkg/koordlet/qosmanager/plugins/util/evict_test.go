...
func Test_DefaultEvictionExecutor(t *testing.T) {
	// test IsPodEvicted
	evictor := framework.NewEvictor(nil, nil, "")
	...
	evictor = framework.NewEvictor(nil, fakeRecorder, "")
	pod := &corev1.Pod{
		Spec: corev1.PodSpec{
...
func Test_InitializeEvictionExecutor(t *testing.T) {
	...
	customExecutorInitializer = func(*framework.Evictor, bool) EvictionExecutor {
