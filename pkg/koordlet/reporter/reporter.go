...
func convertNodeMetricToResourceMap(nodeMetric *metriccache.NodeResourceMetric) *slov1alpha1.ResourceMap {
	...
	if len(nodeMetric.GPUs) > 0 {
		for _, gpu := range nodeMetric.GPUs {
			memoryRatioRaw := 100 * float64(gpu.MemoryUsed.Value()) / float64(gpu.TotalMemory.Value())
			gpuInfo := schedulingv1alpha1.DeviceInfo{
				...
				Resources: map[corev1.ResourceName]resource.Quantity{
					apiext.GPUCore:        *resource.NewQuantity(int64(gpu.SMUtil), resource.DecimalExponent),
					apiext.GPUMemory:      gpu.MemoryUsed,
					apiext.GPUMemoryRatio: *resource.NewQuantity(int64(memoryRatioRaw), resource.DecimalExponent),
				},
			}
