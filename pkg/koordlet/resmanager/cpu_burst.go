...
func (l *burstLimiter) Allow(now time.Time, currentUsageScalePercent int64) (bool, int64) {
	...
	l.currentToken = koordletutil.MaxInt64(koordletutil.MinInt64(l.currentToken, l.bucketCapacity), -l.bucketCapacity)
...
func (b *CPUBurst) getNodeStateForBurst(sharePoolThresholdPercent int64,
	...
	overloadMetricDurationSeconds := koordletutil.MinInt64(int64(b.resmanager.config.ReconcileIntervalSeconds*5), 10)
	...
	for _, podMeta := range podsMeta {
		...
		if podQOS == apiext.QoSLSR {
			podRequest := koordletutil.GetPodRequest(podMeta.Pod)
	...
	nodeBurstState := nodeBurstUnknown
...
func (b *CPUBurst) applyCFSQuotaBurst(burstCfg *slov1alpha1.CPUBurstConfig, podMeta *statesinformer.PodMeta,
	...
	for i := range pod.Status.ContainerStatuses {
		...
		containerTargetCFS = koordletutil.MaxInt64(containerBaseCFS, koordletutil.MinInt64(containerTargetCFS, containerCeilCFS))
...
func (b *CPUBurst) cfsBurstAllowedByLimiter(burstCfg *slov1alpha1.CPUBurstConfig, container *corev1.Container,
	...
	containerCPULimit := float64(koordletutil.GetContainerMilliCPULimit(container)) / 1000
...
func (b *CPUBurst) applyContainerCFSQuota(podMeta *statesinformer.PodMeta, containerStat *corev1.ContainerStatus,
	...
	if deltaContainerCFS > 0 {
		...
		if curPodCFS > 0 {
			...
			updater := NewCommonCgroupResourceUpdater(ownerRef, podDir, sysutil.CPUCFSQuota, podCFSValStr)
		...
		updater := NewCommonCgroupResourceUpdater(ownerRef, containerDir, sysutil.CPUCFSQuota, containerCFSValStr)
		if err := b.executor.Update(updater); err != nil {
...
func (b *CPUBurst) applyCPUBurst(burstCfg *slov1alpha1.CPUBurstConfig, podMeta *statesinformer.PodMeta) {
	...
	for i := range pod.Status.ContainerStatuses {
		...
		if sysutil.ValidateCgroupValue(&containerCFSBurstVal, containerDir, sysutil.CPUBurst) {
			...
			updater := NewCommonCgroupResourceUpdater(ownerRef, containerDir, sysutil.CPUBurst, containerCFSBurstValStr)
	...
	podDir := koordletutil.GetPodCgroupDirWithKube(podMeta.CgroupDir)
	if sysutil.ValidateCgroupValue(&podCFSBurstVal, podDir, sysutil.CPUBurst) {
		ownerRef := PodOwnerRef(pod.Namespace, pod.Name)
		podCFSBurstValStr := strconv.FormatInt(podCFSBurstVal, 10)
		updater := NewCommonCgroupResourceUpdater(ownerRef, podDir, sysutil.CPUBurst, podCFSBurstValStr)
		updated, err := b.executor.UpdateByCache(updater)
		if err != nil {
...
func calcStaticCPUBurstVal(container *corev1.Container, burstCfg *slov1alpha1.CPUBurstConfig) int64 {
	...
	containerCPUMilliLimit := koordletutil.GetContainerMilliCPULimit(container)
	...
	containerCFSBurstVal := int64(cpuCoresBurst * float64(sysutil.CFSBasePeriodValue))
