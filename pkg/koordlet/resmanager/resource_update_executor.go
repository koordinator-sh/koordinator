...
var _ LeveledCacheExecutor = &LeveledResourceUpdateExecutor{}

type CacheExecutor interface {
	UpdateBatchByCache(resources ...ResourceUpdater) (updated bool)
	UpdateBatch(resources ...ResourceUpdater)
...
func (rm *ResourceUpdateExecutor) needUpdate(currentResource ResourceUpdater) bool {
	...
	if time.Now().Sub(preResourceUpdater.GetLastUpdateTimestamp()) > time.Duration(rm.forceUpdateSeconds)*time.Second {
	...
	for i := 0; i < len(resources); i++ {
		for _, resource := range resources[i] {
			...
			if err == nil {
				...
				klog.Errorf("LeveledResourceUpdateExecutor merge update resource %v fail! error: %v", resource.Key(), err)
			}
		}
	}
