...
func (rr *ResctrlRDTReader) ReadResctrlL3Stat(parent string) (map[CacheId]uint64, error) {
	...
	for _, domain := range domains {
		...
		path := system.ResctrlLLCOccupancy.Path(filepath.Join(monDataPath, domain.Name()))
...
func (rr *ResctrlRDTReader) ReadResctrlMBStat(parent string) (map[CacheId]system.MBStatData, error) {
	...
	for _, domain := range domains {
		...
		for _, contentName := range []string{
			system.ResctrlMBLocal.Path(filepath.Join(parent, domain.Name())),
			system.ResctrlMBTotal.Path(filepath.Join(parent, domain.Name())),
		} {
			...
			mbStat[CacheId(cacheId)][contentName] = mbUsage
		}
	}
	return mbStat, nil
}
