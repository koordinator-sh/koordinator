...
func injectCPUShares(cgroupParent string, cpuShares int64) error {
	...
	updater, err := resourceexecutor.GetCgroupUpdater(sysutil.CPUSharesName, cgroupParent, cpuShareStr)
	if err != nil {
		return err
	}
...
func injectCPUSet(cgroupParent string, cpuset string) error {
	updater, err := resourceexecutor.GetCgroupUpdater(sysutil.CPUSetCPUSName, cgroupParent, cpuset)
	if err != nil {
		return err
	}
	return updater.Update()
...
func injectCPUQuota(cgroupParent string, cpuQuota int64) error {
	...
	updater, err := resourceexecutor.GetCgroupUpdater(sysutil.CPUCFSQuotaName, cgroupParent, cpuQuotaStr)
	if err != nil {
		return err
	}
	return updater.Update()
...
func injectMemoryLimit(cgroupParent string, memoryLimit int64) error {
	...
	updater, err := resourceexecutor.GetCgroupUpdater(sysutil.MemoryLimitName, cgroupParent, memoryLimitStr)
	if err != nil {
		return err
	}
	return updater.Update()
...
func injectCPUBvt(cgroupParent string, bvtValue int64) error {
	...
	updater, err := resourceexecutor.GetCgroupUpdater(sysutil.CPUBVTWarpNsName, cgroupParent, bvtValueStr)
	if err != nil {
		return err
	}
	return updater.Update()
