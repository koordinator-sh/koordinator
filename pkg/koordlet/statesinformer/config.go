...
func (c *Config) InitFlags(fs *flag.FlagSet) {
	...
	fs.DurationVar(&c.CRISyncInterval, "cri-sync-interval", c.CRISyncInterval, "The interval at which Koordlet will retain data from CRI. Non-zero values should contain a corresponding time unit (e.g. 1s, 2m, 3h).")
	fs.DurationVar(&c.CRISyncTimeout, "cri-sync-timeout", c.CRISyncTimeout, "The length of time to wait before giving up on a single request to CRI. Non-zero values should contain a corresponding time unit (e.g. 1s, 2m, 3h).")
