...
func (s *statesInformer) reportDevice() {
	...
	func() {
		//if xpu devices exist, report xpu devices; else report gpu devices
		if skipBuildGPU {
			return
		}
...
func (s *statesInformer) updateDevice(device *schedulingv1alpha1.Device) error {
	...
	return util.RetryOnConflictOrTooManyRequests(func() error {
		...
		oldDevConditionMap := make(map[string][]metav1.Condition)
		for _, oldDev := range latestDevice.Spec.Devices {
			oldDevConditionMap[oldDev.UUID] = oldDev.Conditions
		}

		for i, newDev := range device.Spec.Devices {
			if oldDevCondition, ok := oldDevConditionMap[newDev.UUID]; ok {
				updatedConditions := oldDevCondition
				for _, newDevCondition := range newDev.Conditions {
					updatedConditions = setConditionInList(updatedConditions,
						newDevCondition.Type,
						newDevCondition.Status,
						newDevCondition.Reason,
						newDevCondition.Message)
				}

				device.Spec.Devices[i].Conditions = updatedConditions
			}
		}
...
func setConditionInList(inputList []metav1.Condition, conditionType string, status metav1.ConditionStatus, reason, message string) []metav1.Condition {
	resList := inputList
