...
func TestNewPouchRuntimeHandler(t *testing.T) {
	...
	helper.WriteFileContents("/var/run/pouchcri.sock", "test")
	system.Conf.VarRunRootDir = filepath.Join(helper.TempDir, "/var/run")
	PouchEndpoint := GetPouchEndpoint()
	unixEndPoint := fmt.Sprintf("unix://%s", PouchEndpoint)
	pouchRuntime, err := NewPouchRuntimeHandler(unixEndPoint)
	assert.NoError(t, err)
	assert.NotNil(t, pouchRuntime)

	// custom VarRunRootDir
	helper.WriteFileContents("/host-var-run/pouchcri.sock", "test1")
	system.Conf.VarRunRootDir = filepath.Join(helper.TempDir, "/host-var-run")
	PouchEndpoint = GetPouchEndpoint()
	unixEndPoint = fmt.Sprintf("unix://%s", PouchEndpoint)
	pouchRuntime, err = NewContainerdRuntimeHandler(unixEndPoint)
	assert.NoError(t, err)
	assert.NotNil(t, pouchRuntime)
}

func TestPouchRuntimeHandler_StopContainer(t *testing.T) {
	type args struct {
		name         string
		containerId  string
		runtimeError error
...
func TestPouchRuntimeHandler_UpdateContainerResources(t *testing.T) {
	type args struct {
		name         string
		containerId  string
		runtimeError error
