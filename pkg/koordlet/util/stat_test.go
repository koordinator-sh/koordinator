...
func Test_GetCPUStatUsageTicks(t *testing.T) {
	...
	sysutil.SetConf(*sysutil.NewHostModeConfig())
...
func Test_readPodCPUStatUsageTicks(t *testing.T) {
	...
	tempPodStatPath := filepath.Join(tempDir, sysutil.CpuacctStatFileName)
...
func Test_GetRootCgroupCPUStatUsageTicks(t *testing.T) {
	...
	helper.WriteCgroupFileContents(GetKubeQosRelativePath(corev1.PodQOSBestEffort), sysutil.CpuacctStat, getStatContents())
...
func TestGetContainerCPUStatUsageTicks(t *testing.T) {
	...
	helper.WriteCgroupFileContents(containerDir, sysutil.CpuacctStat, getStatContents())
