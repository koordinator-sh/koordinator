...
func CPUIdentityEnabled() (bool, error) {
	rootBvtFilePath := GetCgroupFilePath("/", CPUBVTWarpNs)
	_, err := os.Stat(rootBvtFilePath)
	if err == nil {
		// file exist, cpu identity enabled
		return true, nil
	} else if os.IsNotExist(err) {
		// file not exist, cpu identity disabled
		return false, nil
	} else {
		return false, fmt.Errorf("check cpu identity enabled failed %v", err)
	}
}
