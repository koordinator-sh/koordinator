...
func Test_ReadCatL3Cbm(t *testing.T) {
	type fields struct {
		cbmStr      string
		invalidPath bool
	}
	tests := []struct {
		...
		{
			name:    "cbm is not an unsigned integer",
			fields:  fields{cbmStr: "-1"},
			want:    0,
			wantErr: true,
		},
		{
			name:    "cbm is too large",
			fields:  fields{cbmStr: "99999999999"},
			want:    0,
			wantErr: true,
		},
		{
			name:    "parse cbm successfully",
			fields:  fields{cbmStr: "ff"},
			want:    255,
			wantErr: false,
		},
		{
			name:    "parse cbm successfully 1",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			sysFSRootDir, _ = ioutil.TempDir("", "ReadCatL3Cbm")
