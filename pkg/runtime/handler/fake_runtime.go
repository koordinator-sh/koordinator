...
func (f *FakeRuntimeHandler) UpdateContainerResources(containerID string, opts UpdateOptions) error {
	if opts.CpusetCpus != "" {
		podMeta := f.PodMetas[containerID]
		containerStatus := util.FindContainerStatusByID(podMeta.Pod, containerID)
		if containerStatus == nil {
			return fmt.Errorf("can not findContainerStatusByID! containerId:%s", containerID)
		}
		containerCgroupDir, err := util.GetContainerCgroupPathWithKube(podMeta.CgroupDir, containerStatus)
		if err != nil {
			return fmt.Errorf("GetContainerCgroupPathWithKube error! podId:%v, containerID:%v, error: %v", podMeta.Pod.UID, containerID, err)
		}
		return system.CgroupFileWrite(containerCgroupDir, system.CPUSet, opts.CpusetCpus)
	}
	return nil
}
