...
type Tree interface {
	AddNode(*corev1.Node)
	UpdateNode(*corev1.Node, *corev1.Node)
	DeleteNode(*corev1.Node)
	GetSnapshot() *TreeSnapshot
}

type RebuildableTree interface {
	Tree
	Rebuild()
}
...
func NewTree(clusterNetworkTopology *schedulingv1alpha1.ClusterNetworkTopology) (RebuildableTree, error) {
...
func sortNetworkTopologySpecFromParentToChild(networkTopologySpec []schedulingv1alpha1.NetworkTopologySpec) ([]schedulingv1alpha1.NetworkTopologySpec, error) {
	...
	for _, spec := range networkTopologySpec {
...
type TreeNode struct {
	...
	OfferSlot            int
	DimensionalScoreList []*DimensionScore
}
...
func (t *tree) UpdateNode(oldNode, newNode *corev1.Node) {
	treeNodesMeta, err := t.getTreeNodesMeta(newNode)
	if err != nil {
		klog.Error(err)
		return
	}
	...
	t.deleteNode(oldNode)
	t.addNode(treeNodesMeta, newNode)
}
...
func (t *tree) DeleteNode(node *corev1.Node) {
	t.lock.Lock()
	defer t.lock.Unlock()
	t.deleteNode(node)
}
...
func (t *tree) deleteNode(node *corev1.Node) {
	...
	if treeNode, ok := t.index[meta]; ok {

		delete(t.index, meta)
		delete(treeNode.Parent.Children, meta.Name)
	}
}
