...
func (pgMgr *PodGroupManager) AllowGangGroup(pod *corev1.Pod, handle framework.Handle, pluginName string) {
	...
	if gang.isGangOnceResourceSatisfied() {
		klog.V(5).InfoS("AllowGangGroup: gang is OnceSatisfied, skip recording binding info", "pod",
			klog.KObj(pod), "gang", gang.Name, "gangGroup", gang.GangGroupId)
		gang.clearBindingMembers()
	} else {
		...
		klog.V(4).InfoS("AllowGangGroup: record binding members for gang", "pod",
			klog.KObj(pod), "gang", gang.Name, "gangGroup", gang.GangGroupId, "memberPods", memberPods.Len())
	}
