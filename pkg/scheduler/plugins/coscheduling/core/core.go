...
type Manager interface {
	NextPod() *corev1.Pod
	StartGangScheduling(pod *corev1.Pod)
...
func (pgMgr *PodGroupManager) NextPod() *corev1.Pod {
	...
	if gang == nil {
		...
		pgMgr.rejectGangGroup(pgMgr.handle, gangSchedulingContext.gangGroup, ReasonGangIsNil, nil)
	...
	for _, groupGangId := range gangGroup {
		...
		for _, pod := range pods {
			if !gangSchedulingContext.alreadyFailedPods.Has(util.GetId(pod.Namespace, pod.Name)) {
				// correct podInfo.Time and podInfo.Attempts
				return frameworkext.CopyQueueInfoToPod(firstPod, pod)
			}
		}
	}
	pgMgr.rejectGangGroup(pgMgr.handle, gangSchedulingContext.gangGroup, ReasonAllPendingPodsIsAlreadyAttempted, nil)
	pgMgr.holder.clearGangSchedulingContext(ReasonAllPendingPodsIsAlreadyAttempted)

	return nil
}

func (pgMgr *PodGroupManager) StartGangScheduling(pod *corev1.Pod) {
	gangSchedulingContext := pgMgr.holder.getCurrentGangSchedulingContext()
	if gangSchedulingContext != nil {
		return
	}
	...
	pgMgr.holder.setGangSchedulingContext(gangSchedulingContext, ReasonFirstPodIsWaitingInPermit)
}
...
func (pgMgr *PodGroupManager) PostFilter(ctx context.Context, state *framework.CycleState, pod *corev1.Pod, handle framework.Handle, pluginName string, filteredNodeStatusMap framework.NodeToStatusMap) (*framework.PostFilterResult, *framework.Status) {
	...
	gangSchedulingContext := pgMgr.holder.getCurrentGangSchedulingContext()
	if gangSchedulingContext == nil {
		// first pod failed because of filter check
		gangSchedulingContext = &GangSchedulingContext{firstPod: pod, gangGroup: sets.New[string](gang.GangGroup...)}
		pgMgr.holder.setGangSchedulingContext(gangSchedulingContext, ReasonRecordFirstPodFailureDuringPostFilter)

	}
	if len(gangSchedulingContext.alreadyFailedPods) == 0 {
		gang.ReplaceRepresentative(pod, ReasonGangGroupFailureCauseThisPod)
	}
	gangSchedulingContext.alreadyFailedPods.Insert(util.GetId(pod.Namespace, pod.Name))
	...
	if gang.getGangMode() == extension.GangModeStrict {
		...
		pgMgr.rejectGangGroupById(handle, pluginName, gang.Name, message, gangSchedulingContext)
		return &framework.PostFilterResult{}, framework.NewStatus(framework.Unschedulable,
...
func (pgMgr *PodGroupManager) Unreserve(ctx context.Context, state *framework.CycleState, pod *corev1.Pod, nodeName string, handle framework.Handle, pluginName string) {
	...
	// TODO fix first pod failed in reserve stage, now we just let it go
	...
	if !(gang.getGangMatchPolicy() == extension.GangMatchPolicyOnceSatisfied && gang.isGangOnceResourceSatisfied()) &&
		...
		pgMgr.rejectGangGroupById(handle, pluginName, gang.Name, message, nil)
	}
}

func (pgMgr *PodGroupManager) rejectGangGroupById(handle framework.Handle, pluginName, gangId, message string, gangSchedulingContext *GangSchedulingContext) {
	...
	pgMgr.rejectGangGroup(handle, gangSet, message, gangSchedulingContext)
}

func (pgMgr *PodGroupManager) rejectGangGroup(handle framework.Handle, gangSet sets.Set[string], message string, gangSchedulingContext *GangSchedulingContext) {
	if handle != nil {
		handle.IterateOverWaitingPods(func(waitingPod framework.WaitingPod) {
			...
			if gangSet.Has(waitingGangId) {
				...
				if gangSchedulingContext != nil {
					gangSchedulingContext.alreadyFailedPods.Insert(util.GetId(waitingPod.GetPod().Namespace, waitingPod.GetPod().Name))
				}
