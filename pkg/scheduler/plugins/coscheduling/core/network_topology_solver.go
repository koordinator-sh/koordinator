...
const (
	MessageNoCandidateTopologyNodes = "no candidate topology nodes can accommodate job, %s, %s"
)
...
type NetworkTopologySolver interface {
	...
	PlacePods(
		...
		addPod podFunc,
		jobNetworkRequirements *networktopology.JobTopologyRequirements,
		clusterNetworkTopology *networktopology.TreeNode,
		nodeToScoreList []networktopology.DimensionalNodeToScore,
...
func (solver *networkTopologySolverImpl) PlacePods(
	...
	topologyState := networktopology.TopologyStateFromContext(ctx)
	...
	evaluateTopologyNode(nodeLayeredTopologyNodes, nodeOfferSlot, nodeToScoreList)
	...
	if len(candidateTopologyNodes) == 0 {
		...
		return nil, framework.NewStatus(framework.Unschedulable, fmt.Sprintf(MessageNoCandidateTopologyNodes, strings.Join(reasons, ";"), fitError.Error()))
	}
	...
	for len(layeredTopologyNodes) > 0 {
		...
		for _, layeredTopologyNode := range layeredTopologyNodes {
			if layeredTopologyNode.Layer == schedulingv1alpha1.NodeTopologyLayer {
				nodeToTopologyNodes[layeredTopologyNode.Name] = clusterNetworkTopology
				continue
			}
...
func evaluateTopologyNode(
	...
	for nodeName, offerSlot := range nodeOfferSlot {
		...
		for topologyNode != nil {
			...
			for i, nodeToScore := range nodeToScoreList {
				if len(topologyNode.DimensionalScoreList) == 0 {
					topologyNode.DimensionalScoreList = make([]*networktopology.DimensionScore, len(nodeToScoreList))
				}
				if topologyNode.DimensionalScoreList[i] == nil {
					topologyNode.DimensionalScoreList[i] = &networktopology.DimensionScore{Dimension: nodeToScore.Dimension, PreferLowerScore: nodeToScore.PreferLowerScore}
				}
				topologyNode.DimensionalScoreList[i].Score += nodeToScore.NodeToScore[nodeName]
			}
...
var topologyNodeLessFunc = func(a, b *networktopology.TreeNode, lowerOfferSlot bool) bool {
	...
	for dimensionIndex, dimensionScore := range a.DimensionalScoreList {
		dimensionJScore := b.DimensionalScoreList[dimensionIndex].Score
		if dimensionScore.Score != dimensionJScore {
			if dimensionScore.PreferLowerScore {
				return dimensionScore.Score < dimensionJScore
			} else {
				return dimensionScore.Score > dimensionJScore
			}
		}
...
func distributePods(
	...
	for _, pod := range toSchedulePods {
		...
		if offerSlot > 0 {
			...
			nodeToOfferSlot[currentNode] = offerSlot - 1
			continue
		}
		currentNodeIndex += 1
	}
