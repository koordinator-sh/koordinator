...
func (pgMgr *PodGroupManager) PreFilter(ctx context.Context, state *framework.CycleState, pod *corev1.Pod) (*framework.PreFilterResult, *framework.Status) {
	...
	if len(gangSchedulingContext.alreadyAttemptedPods) > 1 {
		plannedNode := gangSchedulingContext.networkTopologyPlannedNodes[util.GetId(pod.Namespace, pod.Name)]
		if plannedNode == "" {
...
func (pgMgr *PodGroupManager) FindOneNode(ctx context.Context, cycleState *framework.CycleState, pod *corev1.Pod, preRes *framework.PreFilterResult) (string, *framework.Status) {
	...
	topologyState := &networktopology.TopologyState{
		JobTopologyRequirements: &networktopology.JobTopologyRequirements{
			TopologyLayerMustGather: networktopology.GetMustGatherLayer(gangSchedulingContext.networkTopologySpec, gangSchedulingContext.networkTopologySnapshot.IsAncestor),
	...
	ctx = networktopology.ContextWithTopologyState(ctx, topologyState)
	...
	plannedNodes, status := pgMgr.networkTopologySolver.PlacePods(
		...
		gangSchedulingContext.networkTopologySnapshot.TreeNode,
		nil,
	)
	if !status.IsSuccess() {
		return "", status
...
func (ev *preemptionEvaluatorImpl) PlanNodes(
	...
	topologyState := &networktopology.TopologyState{
		JobTopologyRequirements: &networktopology.JobTopologyRequirements{
			TopologyLayerMustGather: networktopology.GetMustGatherLayer(networkTopologySpec, preemptionState.gangSchedulingContext.networkTopologySnapshot.IsAncestor),
			DesiredOfferSlot:        len(allPendingPods),
		},
	}
	ctx = networktopology.ContextWithTopologyState(ctx, topologyState)
