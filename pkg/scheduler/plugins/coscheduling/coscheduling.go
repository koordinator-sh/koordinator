...
func (cs *Coscheduling) Less(podInfo1, podInfo2 *framework.QueuedPodInfo) bool {
	...
	if waitingPodNum1 != 0 || waitingPodNum2 != 0 {
		...
		/*
			In some cases, two gang group may both have some assumed sibling pods.
			For example:
				...
				4. GroupB submit the sixth pod
			Then waitingPodNum will make no sense, so just sort it by group id to give fixed order.
