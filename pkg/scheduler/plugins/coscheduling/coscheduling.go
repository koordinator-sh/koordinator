...
func (cs *Coscheduling) Less(podInfo1, podInfo2 *framework.QueuedPodInfo) bool {
	...
	lastScheduleTime1 := getLastScheduleTime(podInfo1.Pod, podInfo1.Timestamp)
	lastScheduleTime2 := getLastScheduleTime(podInfo2.Pod, podInfo2.Timestamp)
	if lastScheduleTime1 != lastScheduleTime2 {
		return lastScheduleTime1 < lastScheduleTime2
	}
...
func getLastScheduleTime(pod *v1.Pod, podLastScheduleTime time.Time) string {
	if gangGroupLastScheduleTime, ok := pod.Annotations[extension.AnnotationLastScheduleTime]; ok {
		return gangGroupLastScheduleTime
	}
	return podLastScheduleTime.String()
}
