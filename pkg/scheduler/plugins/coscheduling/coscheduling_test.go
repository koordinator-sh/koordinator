...
func TestFairness(t *testing.T) {
	...
	var scheduleOrder []*podScheduleInfo
	...
	assert.GreaterOrEqual(t, 2, maxGangSchedulingCycle-minGangSchedulingCycle)
...
func (p *podScheduleInfo) String() string {
...
func simulateScheduleOne(t *testing.T, ctx context.Context, sched *scheduler.Scheduler, suit *pluginTestSuit, scheduleOrder *[]*podScheduleInfo, injectFilterErr func(pod *corev1.Pod) bool) {
	podInfo := sched.NextPod()
	clonedPodInfo := podInfo.DeepCopy()
	pod := clonedPodInfo.Pod

	scheduleInfo := &podScheduleInfo{
		podID:              pod.Name,
	...
	if exists {
		...
		scheduleInfo.lastScheduleTime = getLastScheduleTime(pod, time.Time{})
	}
	*scheduleOrder = append(*scheduleOrder, scheduleInfo)
	...
	assumedPodInfo := clonedPodInfo.DeepCopy()
...
func schedulePod(ctx context.Context, fwk framework.Framework, state *framework.CycleState, pod *corev1.Pod, info *podScheduleInfo, injectFilterError bool) (result scheduler.ScheduleResult, err error) {
