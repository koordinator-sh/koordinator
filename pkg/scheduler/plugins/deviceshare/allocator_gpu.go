...
const (
	...
	ErrUnsupportedGPULinkType               = "node(s) Unsupported GPU Link Type"
...
func (a *GPUAllocator) Allocate(requestCtx *requestContext, nodeDevice *nodeDevice, desiredCount int, maxDesiredCount int, preferredPCIEs sets.String) ([]*apiext.DeviceAllocation, *framework.Status) {
	...
	allocateContext := &AllocateContext{
		...
		deviceTotal:          nodeDevice.deviceTotal[schedulingv1alpha1.GPU],
...
type GPUPartitionIndexer map[int]map[apiext.GPULinkType][]*PartitionsOfAllocationScore
...
func allocateByPartition(honorGPUPartition bool, gpuRequirements *GPURequirements, gpuPartitionIndexer GPUPartitionIndexer, allocateContext *AllocateContext) (allocations []*apiext.DeviceAllocation, status *framework.Status) {
	...
	indexerOfLinkType, ok := gpuPartitionIndexer[gpuRequirements.numberOfGPUs]
	if !ok || indexerOfLinkType == nil {
		return nil, framework.NewStatus(framework.UnschedulableAndUnresolvable, ErrUnsupportedGPURequests)
	}
	indexerOfAllocationScore, ok := indexerOfLinkType[gpuRequirements.gpuLinkType]
	if !ok || len(indexerOfAllocationScore) == 0 {
		return nil, framework.NewStatus(framework.UnschedulableAndUnresolvable, ErrUnsupportedGPULinkType)
	}
	...
	for _, candidatePartitions := range indexerOfAllocationScore {
		for _, partition := range candidatePartitions.Partitions {
			...
			if gpuRequirements.rindAllReduceBandwidth != nil {
				if partition.RingAllReduceBandwidth == nil {
					continue
				}
				if gpuRequirements.rindAllReduceBandwidth.Cmp(*partition.RingAllReduceBandwidth) > 0 {
					continue
				}
			}
...
func selectPartitionByBinPack(deviceUsedMinorsHash int, feasiblePartitions []*apiext.GPUPartition, partitionIndexer GPUPartitionIndexer, desiredNumberOfGPU int) *apiext.GPUPartition {
	...
	for _, feasiblePartition := range feasiblePartitions {
		...
		for _, numberOfGPUs := range listOfNumberOfGPU {
			...
			if !ok || indexerOfGPUNumber == nil {
				continue
			}
			for _, indexerOfLinkType := range indexerOfGPUNumber {
				if indexerOfLinkType == nil {
					continue
				}
				for _, futurePartition := range indexerOfLinkType {
					for _, partition := range futurePartition.Partitions {
						if partition.MinorsHash&allocatedMinorsHash > 0 {
							continue
						}
						score += scoreOfNumOfGPUs[numberOfGPUs] * futurePartition.AllocationScore
					}
				}
			}
		}
