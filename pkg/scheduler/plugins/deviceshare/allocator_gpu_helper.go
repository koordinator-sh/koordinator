...
var (
	gpuPartitionIndexOfNVIDIAHopper = GPUPartitionIndexer{
		1: map[apiext.GPULinkType][]*PartitionsOfAllocationScore{
			"": {
				{
		...
		2: map[apiext.GPULinkType][]*PartitionsOfAllocationScore{
			"": {
				{
					Partitions: []*apiext.GPUPartition{
						{
		...
		4: map[apiext.GPULinkType][]*PartitionsOfAllocationScore{
			"": {
				{
					Partitions: []*apiext.GPUPartition{
						{
		...
		8: map[apiext.GPULinkType][]*PartitionsOfAllocationScore{
			"": {
				{
					Partitions: []*apiext.GPUPartition{
						{
...
func GetGPUPartitionIndexer(table apiext.GPUPartitionTable) GPUPartitionIndexer {
	...
	gpuPartitionIndexerHelper := make(map[int]map[apiext.GPULinkType]map[int]*PartitionsOfAllocationScore, len(table))
	for numberOfGPUs, tableOfLinkType := range table {
		for linkType, partitions := range tableOfLinkType {
			for i, partition := range partitions {
				if _, ok := gpuPartitionIndexerHelper[numberOfGPUs]; !ok {
					gpuPartitionIndexerHelper[numberOfGPUs] = make(map[apiext.GPULinkType]map[int]*PartitionsOfAllocationScore)
				}
				if _, ok := gpuPartitionIndexerHelper[numberOfGPUs][linkType]; !ok {
					gpuPartitionIndexerHelper[numberOfGPUs][linkType] = make(map[int]*PartitionsOfAllocationScore, 0)
				}
				indexerOfAllocationScore := gpuPartitionIndexerHelper[numberOfGPUs][linkType]
	...
	gpuPartitionIndexer := make(map[int]map[apiext.GPULinkType][]*PartitionsOfAllocationScore, len(table))
	for numberOfGPUs, tableOfLinkType := range gpuPartitionIndexerHelper {
		for linkType, indexerOfAllocationScore := range tableOfLinkType {
			if _, ok := gpuPartitionIndexer[numberOfGPUs]; !ok {
				gpuPartitionIndexer[numberOfGPUs] = make(map[apiext.GPULinkType][]*PartitionsOfAllocationScore)
			}
			if _, ok := gpuPartitionIndexer[numberOfGPUs][linkType]; !ok {
				gpuPartitionIndexer[numberOfGPUs][linkType] = make([]*PartitionsOfAllocationScore, 0)
			}
			for _, partitionsOfAllocationScore := range indexerOfAllocationScore {
				gpuPartitionIndexer[numberOfGPUs][linkType] = append(gpuPartitionIndexer[numberOfGPUs][linkType], partitionsOfAllocationScore)
			}
			sort.Slice(gpuPartitionIndexer[numberOfGPUs][linkType], func(i, j int) bool {
				return gpuPartitionIndexer[numberOfGPUs][linkType][i].AllocationScore < gpuPartitionIndexer[numberOfGPUs][linkType][j].AllocationScore
			})
		}
	}
