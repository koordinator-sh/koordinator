...
func TestController_Run(t *testing.T) {
	...
	}{
		{
			...
			pods: []*v1.Pod{
				...
				MakePod("t1-ns1", "pod2").Phase(v1.PodPending).Container(
					MakeResourceList().CPU(1).Mem(2).GPU(0).Obj()).Obj(),
			},
			want: []*v1alpha1.ElasticQuota{
		...
		},
		{
			...
			pods: []*v1.Pod{
				...
				MakePod("t2-ns1", "pod1").Phase(v1.PodRunning).
					...
					Container(MakeResourceList().CPU(1).Mem(2).Obj()).Obj(),
				//CPU: 3 Mem: 3
				MakePod("t2-ns1", "pod2").Phase(v1.PodRunning).
					...
					Container(
						MakeResourceList().CPU(2).Mem(1).Obj()).
					Container(
						MakeResourceList().CPU(1).Mem(1).Obj()).Obj(),
			},
			want: []*v1alpha1.ElasticQuota{
				MakeEQ("t2-ns1", "t2-ns1").
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			ctrl := pg.elasticQuotaController
			...
			err := wait.Poll(200*time.Millisecond, 3*time.Second, func() (done bool, err error) {
				for _, v := range c.want {
					get, err := suit.client.SchedulingV1alpha1().ElasticQuotas(v.Namespace).Get(ctx, v.Name, metav1.GetOptions{})
					if err != nil {
						return false, err
					}
