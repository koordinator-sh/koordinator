...
func (gqm *GroupQuotaManager) UpdateQuota(quota *v1alpha1.ElasticQuota, isDelete bool) error {
	...
	if isDelete {
		...
		var oldCustomState *CustomState
		if gqm.customLimitersManager.enabled() {
			quotaInfo := gqm.getQuotaInfoByNameNoLock(quota.Name)
			if quotaInfo == nil {
				return fmt.Errorf("get quota info failed, quotaName:%v", quota.Name)
			}
			topNode := gqm.getTopNodeNoLock(quotaInfo)
			oldCustomState = gqm.customLimitersManager.parseCustomState(quotaInfo, topNode)
		}
...
func (gqm *GroupQuotaManager) updateQuotaInternalNoLock(newQuotaInfo, oldQuotaInfo *QuotaInfo) {
	...
	var oldCustomState *CustomState
	if gqm.customLimitersManager.enabled() {
		// topNode could not be nil
		topNode := gqm.getTopNodeNoLock(newQuotaInfo)
		if topNode != nil {
			// oldQuotaInfo could be nil for newly added quota, not nil for updated quota
			oldCustomState = gqm.customLimitersManager.parseCustomState(oldQuotaInfo, topNode)
		}
	}
...
func (gqm *GroupQuotaManager) updateQuotaNoLockWhenParentChange(newQuota *v1alpha1.ElasticQuota) {
	...
	var oldCustomState *CustomState
	if gqm.customLimitersManager.enabled() {
		topNode := gqm.getTopNodeNoLock(oldQuotaInfo)
		oldCustomState = gqm.customLimitersManager.parseCustomState(oldQuotaInfo, topNode)
	}
