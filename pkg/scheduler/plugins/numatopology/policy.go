...
type Policy interface {
	...
	Merge(providersHints []map[string][]frameworkext.NUMATopologyHint) (frameworkext.NUMATopologyHint, bool)
}

// Merge a TopologyHints permutation to a single hint by performing a bitwise-AND
// of their affinity masks. The hint shall be preferred if all hits in the permutation
// are preferred.
func mergePermutation(numaNodes []int, permutation []frameworkext.NUMATopologyHint) frameworkext.NUMATopologyHint {
	...
	return frameworkext.NUMATopologyHint{mergedAffinity, preferred}
}

func filterProvidersHints(providersHints []map[string][]frameworkext.NUMATopologyHint) [][]frameworkext.NUMATopologyHint {
	// Loop through all hint providers and save an accumulated list of the
	// hints returned by each hint provider. If no hints are provided, assume
	// that provider has no preference for topology-aware allocation.
	var allProviderHints [][]frameworkext.NUMATopologyHint
	for _, hints := range providersHints {
		// If hints is nil, insert a single, preferred any-numa hint into allProviderHints.
		if len(hints) == 0 {
			klog.Infof("[topologymanager] Hint Provider has no preference for NUMA affinity with any resource")
			allProviderHints = append(allProviderHints, []frameworkext.NUMATopologyHint{{nil, true}})
			continue
		}
		...
		for resource := range hints {
			...
			if len(hints[resource]) == 0 {
				...
				allProviderHints = append(allProviderHints, []frameworkext.NUMATopologyHint{{nil, false}})
				continue
			}
...
func mergeFilteredHints(numaNodes []int, filteredHints [][]frameworkext.NUMATopologyHint) frameworkext.NUMATopologyHint {
	...
	bestHint := frameworkext.NUMATopologyHint{defaultAffinity, false}
	iterateAllProviderTopologyHints(filteredHints, func(permutation []frameworkext.NUMATopologyHint) {
		// Get the NUMANodeAffinity from each hint in the permutation and see if any
		// of them encode unpreferred allocations.
...
func iterateAllProviderTopologyHints(allProviderHints [][]frameworkext.NUMATopologyHint, callback func([]frameworkext.NUMATopologyHint)) {
	// Internal helper function to accumulate the permutation before calling the callback.
	var iterate func(i int, accum []frameworkext.NUMATopologyHint)
	iterate = func(i int, accum []frameworkext.NUMATopologyHint) {
	...
	iterate(0, []frameworkext.NUMATopologyHint{})
}
