...
func (p *singleNumaNodePolicy) canAdmitPodResult(hint *frameworkext.NUMATopologyHint) bool {
	return hint.Preferred
}

// Return hints that have valid bitmasks with exactly one bit set.
func filterSingleNumaHints(allResourcesHints [][]frameworkext.NUMATopologyHint) [][]frameworkext.NUMATopologyHint {
	var filteredResourcesHints [][]frameworkext.NUMATopologyHint
	for _, oneResourceHints := range allResourcesHints {
		var filtered []frameworkext.NUMATopologyHint
...
func (p *singleNumaNodePolicy) Merge(providersHints []map[string][]frameworkext.NUMATopologyHint) (frameworkext.NUMATopologyHint, bool) {
	...
	if bestHint.NUMANodeAffinity.IsEqual(defaultAffinity) {
		bestHint = frameworkext.NUMATopologyHint{nil, bestHint.Preferred}
	}
