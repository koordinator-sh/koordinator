...
func TestReservationCacheIndexAddRemoveAndUpdate(t *testing.T) {
	...
	// Multi-prefix selector: returns the smallest bucket as a coarse filter.
	// Both prefix buckets have 2 nodes each, so the result is whichever bucket
	// is picked first. The true match (node-a) is always included; downstream
	// checkReservationMatchedOrIgnored does the precise per-reservation match.
...
func TestReservationCacheIndexExactKey(t *testing.T) {
	...
	// Multi-exact-key selector: returns the smallest bucket. project[x] has
	// 1 node (node-a), team[alpha] has 2 nodes (node-a, node-c). The smallest
	// is project[x] => {node-a}, which happens to be the precise answer.
	nodes, hit = c.FilterByReservationSelector(map[string]string{idxExactTeam: "alpha", idxExactProject: "x"})
	assert.True(t, hit)
	assert.ElementsMatch(t, []string{"node-a"}, nodes)

	// Both exact buckets have 1 node each (team[beta]={node-b}, project[x]={node-a});
	// the smallest-bucket strategy returns one of them (non-deterministic). The
	// downstream per-reservation match rejects the false positive.
...
// that the smallest-bucket coarse filter returns a superset of the true
// matches for a mixed prefix + exact selector.
func TestReservationCacheIndexMixedPrefixAndExactKey(t *testing.T) {
	...
	// Mixed prefix + exact selector: returns the smallest bucket. Both buckets
	// have 2 nodes each (quota={node-a,node-b}, team[alpha]={node-a,node-c}),
	// so the result is whichever is picked first. The true match (node-a) is
	// always included.
	nodes, hit = c.FilterByReservationSelector(map[string]string{
		idxPrefixQuota + "x": "v",
		idxExactTeam:         "alpha",
	...
	// Mixed selector with a non-matching exact value: team[gamma] bucket is
	// empty, so the function short-circuits to ([], true) even though the
	// prefix bucket alone would yield {node-a, node-b}.
	nodes, hit = c.FilterByReservationSelector(map[string]string{
		idxPrefixQuota + "x": "v",
		idxExactTeam:         "gamma",
	})
