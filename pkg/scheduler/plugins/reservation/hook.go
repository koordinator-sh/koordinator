...
func prepareMatchReservationState(handle frameworkext.ExtendedHandle, pod *corev1.Pod) (*stateData, error) {
	...
	handle.Parallelizer().Until(context.TODO(), len(allNodes), processNode)
...
func preparePreFilterNodeInfo(nodeInfo *framework.NodeInfo, pod *corev1.Pod, matchedCache *AvailableCache) {
	...
	for _, podInfo := range nodeInfo.PodsWithRequiredAntiAffinity {
		...
		if matchedCache.Get(GetReservePodKey(existingPod)) != nil {
			...
			podInfo.RequiredAntiAffinityTerms = nil
		}
	}
}
