...
const (
	...
	// ErrReasonReservationExpired is the reason for the reservation is expired and should not be used.
	ErrReasonReservationExpired = "reservation is expired"
...
func New(args runtime.Object, handle framework.Handle) (framework.Plugin, error) {
	...
	// index reservation with status.nodeName
...
func (p *Plugin) Reserve(ctx context.Context, cycleState *framework.CycleState, pod *corev1.Pod, nodeName string) *framework.Status {
	...
	for _, rInfo := range rOnNode {
		...
		if r == nil { // reservation not found in active cache, abort
			if p.reservationCache.IsExpired(r) { // in case reservation is marked as expired
...
func (p *Plugin) PreBind(ctx context.Context, cycleState *framework.CycleState, pod *corev1.Pod, nodeName string) *framework.Status {
	...
	err := retryOnConflictOrTooManyRequests(func() error {
		curR, err1 := p.lister.Reservations(target.Namespace).Get(target.Name)
		...
		if !IsReservationScheduled(curR) {
			...
			return fmt.Errorf(ErrReasonReservationExpired)
		}
...
func (p *Plugin) Bind(ctx context.Context, cycleState *framework.CycleState, pod *corev1.Pod, nodeName string) *framework.Status {
	...
	if IsReservePod(pod) {
		klog.V(4).InfoS("Attempting to fake bind reserve pod to node", "pod", klog.KObj(pod), "node", nodeName)
		err := retryOnConflictOrTooManyRequests(func() error {
			// get latest version of the reservation
			r, err1 := p.lister.Reservations(pod.Namespace).Get(pod.Name)
			if errors.IsNotFound(err1) {
				klog.V(3).InfoS("reservation not found, abort the update", "reservation", klog.KObj(r))
				return fmt.Errorf(ErrReasonReservationNotFound)
			} else if err1 != nil {
				klog.Warningf("failed to get reservation %v, err: %v", klog.KObj(r), err1)
				return err1
			}

			// check if the reservation has been expired
			if r.Status.Phase == schedulingv1alpha1.ReservationExpired || p.reservationCache.IsExpired(r) {
				return fmt.Errorf(ErrReasonReservationExpired)
			}
			...
			_, err1 = p.client.Reservations(r.Namespace).UpdateStatus(context.TODO(), r, metav1.UpdateOptions{})
			// TBD: we may just set status.nodeName to avoid multiple updates
			if err1 == nil {
				_, err1 = p.client.Reservations(r.Namespace).Update(context.TODO(), r, metav1.UpdateOptions{})
			} else {
				klog.Warningf("failed to update status for reservation %v, err: %v", klog.KObj(r), err1)
			}
			if err1 == nil {
				return nil
			}
...
func (p *Plugin) syncPodDeleted(pod *corev1.Pod) {
	...
	err = retryOnConflictOrTooManyRequests(func() error {
		r, err1 := p.lister.Reservations(reservationAllocated.Namespace).Get(reservationAllocated.Name)
		if errors.IsNotFound(err1) {
