		...
		status := func() *fwktype.Status {
			...
			if len(state.preemptible[node.Name]) > 0 || len(state.preemptibleInRRs[node.Name]) > 0 {
				...
				if len(insufficientResources) != 0 {
					return fwktype.NewStatus(fwktype.UnschedulableAndUnresolvable, ErrReasonPreemptionFailed)
				}
			}
			return nil
		}()
...
func (pl *Plugin) filterWithReservations(ctx context.Context, cycleState fwktype.CycleState, pod *corev1.Pod, nodeInfo fwktype.NodeInfo, matchedReservations []*frameworkext.ReservationInfo, requiredFromReservation bool) *fwktype.Status {
	...
	if requiredFromReservation {
		...
		if len(allInsufficientResourcesByNode) > 0 {
			...
			nodeAllocatable2 := nodeInfo.GetAllocatable().(*framework.Resource)
			insufficientResourcesByNode := fitsNode(state.podRequestsResources, nodeAllocatable2, nodeRState.podRequested, nodeRState.rAllocated, nil, len(nodeRState.matchedOrIgnored), len(nodeInfo.GetPods()), preemptibleResource)
...
func (pl *Plugin) filterWithPreAllocatablePods(ctx context.Context, cycleState fwktype.CycleState, rInfo *frameworkext.ReservationInfo, nodeInfo fwktype.NodeInfo, preAllocatablePods []*corev1.Pod, isPreAllocationRequired bool) *fwktype.Status {
	extender, ok := pl.handle.(frameworkext.FrameworkExtender)
	if !ok {
		return fwktype.AsStatus(fmt.Errorf("not implemented frameworkext.FrameworkExtender"))
	}
	...
	if isPreAllocationRequired {
		...
		if len(allInsufficientResourcesByNode) > 0 {
			...
			unallocNodeAlloc := nodeInfo.GetAllocatable().(*framework.Resource)
			insufficientResourcesByNodeUnallocated := fitsNode(state.podRequestsResources, unallocNodeAlloc,
				nodeRState.podRequested, nil, nil, 0, len(nodeInfo.GetPods()), preemptible)
