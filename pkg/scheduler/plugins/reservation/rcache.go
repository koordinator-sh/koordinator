...
// reservationCache caches the active and expired reservations synced from informer and plugin reserve.
// returned objects are for read-only usage
type reservationCache struct {
	lock    sync.RWMutex
	expired map[string]*schedulingv1alpha1.Reservation // UID -> *object; expired reservations
...
func (c *reservationCache) AddToExpired(r *schedulingv1alpha1.Reservation) {
	c.lock.Lock()
	defer c.lock.Unlock()
	c.expired[GetReservationKey(r)] = r
	c.active.Delete(r)
}
...
func (c *reservationCache) Delete(r *schedulingv1alpha1.Reservation) {
	...
	defer c.lock.Unlock()
	c.active.Delete(r)
	delete(c.expired, GetReservationKey(r))
}
...
func (c *reservationCache) GetAllExpired() map[string]*schedulingv1alpha1.Reservation {
	c.lock.RLock()
	defer c.lock.RUnlock()
...
func (c *reservationCache) IsExpired(r *schedulingv1alpha1.Reservation) bool {
	c.lock.RLock()
	defer c.lock.RUnlock()
	_, ok := c.expired[GetReservationKey(r)]
	return ok
}
...
func (c *reservationCache) HandleOnUpdate(oldObj, newObj interface{}) {
	...
	if IsReservationActive(newR) {
		c.AddToActive(newR)
	} else if IsReservationExpired(newR) {
		c.AddToExpired(newR)
	}
