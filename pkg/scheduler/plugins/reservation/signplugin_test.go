...
func TestPlugin_SignPod(t *testing.T) {
	...
	// PreFilter narrows a pre-allocation reserve pod to the nodes holding a
	// matching reservation, and the batch hint is applied before that node set
	// is. Keying on the reservation name keeps every reserve pod's signature
	// unique, so a reserve pod never inherits another pod's hint.
	t.Run("reserve pods for different reservations never share a signature", func(t *testing.T) {
		other := reservePod.DeepCopy()
		other.Name, other.UID = "other-rp", types.UID("other-rp")
		other.Annotations[reservationutil.AnnotationReservationName] = "other-r"
		require.NotEqual(t, reservationutil.GetReservationNameFromReservePod(reservePod),
			reservationutil.GetReservationNameFromReservePod(other))

		fa, _ := pl.SignPod(context.TODO(), reservePod)
		fb, _ := pl.SignPod(context.TODO(), other)
