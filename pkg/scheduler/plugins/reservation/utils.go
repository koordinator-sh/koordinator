...
func IsReservationExpired(r *schedulingv1alpha1.Reservation) bool {
	return r != nil && r.Status.Phase == schedulingv1alpha1.ReservationExpired
}
...
func isReservationNeedCleanup(r *schedulingv1alpha1.Reservation) bool {
	return r == nil || IsReservationExpired(r) && (r.Status.Expired == nil ||
		time.Since(r.Status.Expired.Time) > defaultGCDuration)
}
...
func setReservationExpired(r *schedulingv1alpha1.Reservation) {
	r.Status.Phase = schedulingv1alpha1.ReservationExpired
	r.Status.Expired = &metav1.Time{Time: time.Now()}
...
func matchObjectRef(pod *corev1.Pod, objRef *corev1.ObjectReference) bool {
	...
	return objRef == nil ||
		(len(objRef.UID) <= 0 || pod.UID == objRef.UID) &&
			...
			(len(objRef.Kind) <= 0 || pod.Kind == objRef.Kind) &&
