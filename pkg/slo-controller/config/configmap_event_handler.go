...
type EnqueueRequestForConfigMap struct {
	...
	CacheConfigIfChanged func(configMap *v1.ConfigMap) bool
}
...
func (p *EnqueueRequestForConfigMap) Create(evt event.CreateEvent, q workqueue.RateLimitingInterface) {
	...
	if p.CacheConfigIfChanged != nil && !p.CacheConfigIfChanged(configMap) {
		return
	}
	p.EnqueueRequest(&q)
}
...
func (p *EnqueueRequestForConfigMap) Update(evt event.UpdateEvent, q workqueue.RateLimitingInterface) {
	newConfigMap := evt.ObjectNew.(*v1.ConfigMap)
	oldConfigMap := evt.ObjectOld.(*v1.ConfigMap)
	...
	if p.CacheConfigIfChanged != nil && !p.CacheConfigIfChanged(newConfigMap) {
		return
	}
