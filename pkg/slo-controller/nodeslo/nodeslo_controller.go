...
func (r *NodeSLOReconciler) getNodeSLOSpec(node *corev1.Node, oldSpec *slov1alpha1.NodeSLOSpec) (*slov1alpha1.NodeSLOSpec, error) {
	...
	nodeSLOSpec.ResourceUsedThresholdWithBE, _ = getResourceThresholdSpec(node, &sloCfg.ThresholdCfgMerged)

	// resourceQOS spec
	nodeSLOSpec.ResourceQoSStrategy, _ = getResourceQoSSpec(node, &sloCfg.ResourceQoSCfgMerged)

	nodeSLOSpec.CPUBurstStrategy, _ = getCPUBurstConfigSpec(node, &sloCfg.CPUBurstCfgMerged)
...
func (r *NodeSLOReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// if configmap has not got synced, requeue the req
	if !r.sloCfgCache.IsSLOCfgAvailable() {
