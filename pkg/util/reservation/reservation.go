...
// GetReservationSelector returns a defensive copy of the ReservationSelector
// field from the pod's ReservationAffinity. A copy is returned -- rather than
// the internal map -- so that callers cannot mutate the affinity object's
// internal state and introduce data races. Returns nil when no selector was
// specified.
