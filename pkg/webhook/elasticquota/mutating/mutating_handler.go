...
func (h *ElasticQuotaMutatingHandler) Handle(ctx context.Context, request admission.Request) (resp admission.Response) {
	...
	if request.Operation == v1.Update {
		if err := h.Decoder.DecodeRaw(request.OldObject, oldQuota); err != nil {
			return admission.Errored(http.StatusBadRequest, err)
		}
	}
	...
	defer func() {
		if resp.Allowed {
			klog.V(5).Infof("Webhook finish mutating quota %s, allowed: %v, patches: %v",
				obj.Name, resp.Allowed, util.DumpJSON(resp.Patches))
		} else {
			klog.Warningf("Webhook finish mutating quota %s, allowed: %v, result:% v",
				obj.Name, resp.Allowed, util.DumpJSON(resp.Result))
		}
	}()

	for _, pluginFunc := range defaultMutatingPlugins {
		plugin := pluginFunc(h.Decoder, h.Client)
		klog.V(5).Infof("[quota-mutating] quota: %v, plugin: %v, start", obj.Name, plugin.Name())
		if err := plugin.Admit(ctx, request, copied); err != nil {
			return admission.Errored(http.StatusBadRequest, err)
		}
		klog.V(5).Infof("[quota-mutating] quota: %v, plugin: %v, end", obj.Name, plugin.Name())
	}
