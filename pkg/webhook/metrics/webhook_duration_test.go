...
func TestStatusForError(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			err:      fmt.Errorf("wrap error: %w", context.Canceled),
		...
		{
			name:     "server timeout API error",
			err:      apierrors.NewServerTimeout(schema.GroupResource{}, "GET", 0),
			expected: StatusTimeout,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := statusForError(tt.err)
			if got != tt.expected {
				t.Errorf("statusForError() = %v, expected %v", got, tt.expected)
			}
		})
	}
}
